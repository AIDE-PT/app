...
function HeartMiniChart({ data, w, h, isDark, semantic = DEFAULT_SEMANTIC }: { data: number[]; w: number; h: number; isDark: boolean; semantic?: { success: string; warning: string; danger: string } }) {
  ...
  return (
    <Svg width={w} height={h}>
      ...
      {zones.map((z) => {
        const y1 = toY(Math.min(z.hi, mx)); const y2 = toY(Math.max(z.lo, mn));
        if (y2 <= y1) return null;
        return <Rect key={z.lo} x={pad.l} y={y1} width={w - pad.l - pad.r} height={y2 - y1} fill={z.color} opacity={isDark ? 0.12 : 0.22} />;
      })}
      <Path d={fillPath} fill={`url(#${gId})`} />
      <Path d={linePath} fill="none" stroke={BRAND_BLUE} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
      <Circle cx={toX(pts.length - 1)} cy={toY(pts[pts.length - 1])} r={3} fill={BRAND_BLUE} />
    </Svg>
  );
}
...
function TempMiniChart({ value, w, h, isDark, semantic = DEFAULT_SEMANTIC }: { value: number; w: number; h: number; isDark: boolean; semantic?: { success: string; warning: string; danger: string } }) {
  ...
  return (
    <Svg width={w} height={h}>
      ...
      <Rect x={tubeX - tubeW / 2} y={tubeTop} width={tubeW} height={tubeH} rx={tubeW / 2} fill={track} />
      {fillH > 0 && (
        <Rect x={tubeX - tubeW / 2} y={fillY} width={tubeW} height={fillH + bulbR}
          rx={fillH > tubeH * 0.95 ? tubeW / 2 : 2} fill="url(#tmFill)" />
      )}
      <Circle cx={tubeX} cy={tubeBot + bulbR} r={bulbR} fill={fillColor} />
      <Circle cx={tubeX} cy={tubeBot + bulbR} r={bulbR - 3} fill={fillColor} opacity={0.5} />
    </Svg>
  );
}
...
function O2MiniChart({ value, w, h, isDark, semantic = DEFAULT_SEMANTIC }: { value: number; w: number; h: number; isDark: boolean; semantic?: { success: string; warning: string; danger: string } }) {
  ...
  return (
    <Svg width={w} height={h}>
      ...
      <Path d={arcPath(startA, startA + sweepA)} fill="none" stroke={track} strokeWidth={r * 0.16} strokeLinecap="round" />
      {zones.map((z, i) => (
        <Path key={i} d={arcPath(z.from, z.to)} fill="none" stroke={z.color} strokeWidth={r * 0.065} strokeLinecap="butt" opacity={0.7} />
      ))}
      {pct > 0 && (
        <Path d={arcPath(startA, curA)} fill="none" stroke={`url(#${gId})`} strokeWidth={r * 0.16} strokeLinecap="round" />
      )}
...
function SleepMiniChart({ data, w, h, isDark }: { data: number[]; w: number; h: number; isDark: boolean }) {
  const bars = data.slice(-7).reverse(); // Last 7 nights
  if (!bars.length) return null;
  const max = 12; // 12 hours max scale
  const gap = 4;
  const barW = Math.max(Math.floor((w - (bars.length - 1) * gap) / bars.length), 4);
  const toH = (v: number) => (Math.min(v, max) / max) * h;
  const color = (v: number) => v >= 7 && v <= 9 ? "#6366F1" : v < 7 ? "#F59E0B" : "#818CF8";
  ...
  return (
    <Svg width={w} height={h}>
      {bars.map((v, i) => {
        ...
        const x = i * (barW + gap);
        // Rounded top bars
        return <Rect key={i} x={x} y={h - bh} width={barW} height={bh} rx={barW/2} fill={color(v)} opacity={0.8} />;
      })}
      {/* 8h goal line */}
      <Line x1={0} y1={h * (1 - 8/12)} x2={w} y2={h * (1 - 8/12)} stroke={isDark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.2)"} strokeDasharray="4,4" strokeWidth="1" />
    </Svg>
  );
}
...
function GlycemiaMiniChart({ data, w, h, isDark, semantic = DEFAULT_SEMANTIC }: { data: number[]; w: number; h: number; isDark: boolean; semantic?: { success: string; warning: string; danger: string } }) {
  ...
  return (
    <Svg width={w} height={h}>
      ...
      <Circle cx={toX(pts.length - 1)} cy={toY(pts[pts.length - 1])} r={3} fill="#EC4899" />
    </Svg>
  );
}

// 8. BP - Area chart (Simulated range since we only have average)
function BPMiniChart({ data, w, h, isDark }: { data: number[]; w: number; h: number; isDark: boolean }) {
  // data is roughly (sys+dia)/2. 
  // We can just plot this trend line, maybe slightly thicker or different style
    const pts = data.slice(-16);
  if (pts.length < 2) return null;
  const mn = 60; const mx = 140; // Avg BP range
  const pad = { l: 0, r: 0, t: 2, b: 0 };
  const toX = (i: number) => pad.l + (i / (pts.length - 1)) * (w - pad.l - pad.r);
  const toY = (v: number) => pad.t + (1 - (Math.min(Math.max(v, mn), mx) - mn) / (mx - mn)) * (h - pad.t - pad.b);
...
interface DASHBOARD_CONFIGProps {
  id: string; type: IconType; variant: WidgetVariant; value: string; feedback: string; endpoint: string;
}
export const DASHBOARD_CONFIG: DASHBOARD_CONFIGProps[] = [
  { id: "heart",           type: "heartRate",     variant: "1-2", value: "73",     feedback: "normal",  endpoint: "bpm" },
  { id: "steps",           type: "steps",         variant: "1-2", value: "10.432", feedback: "meta",    endpoint: "steps" },
  { id: "blood Pressure",  type: "bloodPressure", variant: "1-3", value: "10.432", feedback: "meta",    endpoint: "bloodPressure" },
  { id: "temp",            type: "temp",          variant: "1-2", value: "36.6",   feedback: "estável", endpoint: "temperature" },
  { id: "sleep",           type: "sleep",         variant: "1-3", value: "7",      feedback: "bom",     endpoint: "sleep" },
  { id: "o2",              type: "o2",            variant: "1-2", value: "98",     feedback: "normal",  endpoint: "o2" },
  { id: "glycemia",        type: "cal",           variant: "1-2", value: "450",    feedback: "meta",    endpoint: "glycemia" },
  { id: "stress",          type: "stress",        variant: "1-2", value: "3",      feedback: "baixo",   endpoint: "stress" },
];
...
export interface DashboardItem { id: string; type: IconType; variant: WidgetVariant; }

//  WidgetWrapper props 
interface WidgetWrapperProps {
  title: string; variant: WidgetVariant;
  children?: React.ReactNode; bg?: string; style?: ViewStyle;
  icon: ReactNode; unit: string; feedback: string; feedbackColor: string;
  value: string; history?: number[]; color?: string;
  metricType?: IconType;
  // legacy (unused but kept so nothing breaks)
  yMin?: number; yMax?: number; segments?: number;
  sparklineHeight?: number; valueOffsetBottom?: number;
}
  ...
  return (
    <View style={[{ width, height, boxShadow: "0 2px 8px 0 rgba(0,0,0,0.12)" }, style]}
      ...
      <View style={{ width: effectiveValueWidth, paddingRight: 4 }} className={metricType === "steps" ? "justify-center" : "justify-between"}>
        ...
        <View className="flex-row items-center gap-1">
          {icon}
          <Text className="text-aide-light-blue text-base font-open-sans-semibold tracking-wide" numberOfLines={1}>{title}</Text>
        </View>
