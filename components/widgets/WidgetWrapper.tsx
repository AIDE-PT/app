...
export function WidgetWrapper({
  ...
  return (
    ...
    >
      ...
      {variant === "1-1" && (
        <>
          <Text className="text-3xl font-bold text-red-500">{value}</Text>

          <Text className="text-xs text-gray-400 font-semibold">{unit}</Text>
        </>
      )}
      {(variant === "1-3" || variant === "1-2") && (
        <View className="flex-row w-full justify-between items-center ml-2">
          {/* ESQUERDA */}
          ...
          {/* DIREITA */}
          <View className="text-xs text-gray-400 mr-2 font-semibold">
            <Text
