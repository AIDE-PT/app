...
const TopTitleNav = ({ title, subtitle, dark = true, href }: TopTitleNavProps) => {
    ...
    return (
        <View className="px-5 pt-4 pb-3 bg-white">
            <View className="flex-row items-center mb-2">
                ...
                <Text className={`font-safiro text-[28px] ${dark ? 'text-[#1A1A2E]' : 'text-white'}`}>
                    {title}
                </Text>
            </View>
            {subtitle && (
                <Text className={`font-open-sans text-[14px] ml-8 ${dark ? 'text-[#9CA3AF]' : 'text-white/60'}`}>
                    {subtitle}
                </Text>
            )}
        </View>
