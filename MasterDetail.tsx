...
const METRIC_CONFIGS: Record<string, MetricConfig> = {
  ...
  glycemia: {
    ...
    accent: "#F97316",
    getStatus: (v) =>
      v >= 70 && v <= 99 ? "normal" : v <= 125 ? "warning" : "alert",
    statusLabel: (s) =>
      s === "normal" ? "Normal" : s === "warning" ? "Pré-Diabético" : "Elevado",
    extraCards: (history) => {
      const avg = history.length
        ? Math.round(history.reduce((a, b) => a + b, 0) / history.length)
        : 0;
...
function buildDays(
  centerDate: Date,
): { day: number; weekday: string; month: string }[] {
...
export default function MasterDetail() {
  ...
  const displayNum =
    typeof currentValue === "number" ? parseFloat(currentValue.toFixed(1)) : 0;
          ...
          <SafeAreaView className="flex-1">
            ...
            {isLoading ? (
              ...
              >
                ...
                <View className="mb-6">
                  <Text
                    className={`text-lg font-open-sans text-center mb-4 ${textPrimary}`}
                  >
                    {days[0].day} a {days[days.length - 1].day} de{" "}
                    {days[0].month}
                  </Text>
                  <ScrollView
                    horizontal
                  ...
                  >
                    {days.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <Text
                            className={`text-xs font-open-sans mb-1 ${isSelected ? textPrimary : textSecondary}`}
                          >
                            {item.weekday}
                          </Text>
                          <Text
                            className={`text-xl font-bold font-open-sans ${textPrimary}`}
                          >
                            {item.day}
                          </Text>
                        </TouchableOpacity>
                ...
                >
                  <View className="flex-row justify-between items-center mb-4">
                    <Text className={`text-lg font-safiro ${textPrimary}`}>
                      Histórico
                    </Text>
                    <Text className={`text-sm font-open-sans ${textSecondary}`}>
                      Últimas 24h
                    </Text>
                  </View>
                  <LineChartSlim
                ...
                <View className="mt-6">
                  <View className="flex-row gap-3">
                    {extraCards.map((card) => (
                      ...
                      >
                        <Text
                          className={`text-sm font-open-sans mb-1 ${textSecondary}`}
                        >
                          {card.label}
                        </Text>
                        <View className="flex-row items-baseline">
                          <Text
                            className={`text-3xl font-bold font-safiro ${textPrimary}`}
                          >
                            {card.value}
                          </Text>
...
function StepsProgressBar({
  ...
  return (
    ...
    >
      ...
      <View
        className={`h-3 rounded-full w-full ${isDark ? "bg-white/10" : "bg-gray-100"}`}
      >
        <View
...
function StressGauge({
  ...
  return (
    <View
      ...
      style={{ boxShadow: "0 2px 8px 0 rgba(0,0,0,0.12)" } as any}
    >
      <Text className={`text-sm font-open-sans mb-3 ${textSecondary}`}>
        Zonas de Stress
      </Text>
      {zones.map((z) => (
        <View key={z.label} className="flex-row items-center mb-2 gap-3">
          ...
          />
          <Text className={`text-sm font-open-sans ${textPrimary}`}>
            {z.label}
          </Text>
          <Text className={`text-sm font-open-sans ml-auto ${textSecondary}`}>
            {z.range}
          </Text>
        </View>
      ))}
      <View
        className={`mt-2 h-[1px] ${isDark ? "bg-white/10" : "bg-gray-100"}`}
      />
      <Text className={`text-xs mt-2 font-open-sans ${textSecondary}`}>
