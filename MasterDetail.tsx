...
export function useMetricStats(endpoint: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const data = await fetchMetricFromSupabaseByTypeNames(typeNames, isBP);
...
export function useHealthMetric(endpoint: string, isBP: boolean = false) {
  return useQuery({
    ...
    refetchOnReconnect: true,
    queryFn: async () => {
      // Passos — Health Connect primeiro, BD como fallback
      if (endpoint === "steps") {
        const healthConnectSteps = await fetchHealthConnectStepsDaily();
        if (healthConnectSteps) return healthConnectSteps;
        return fetchMetricFromSupabaseByTypeNames(TYPE_MAP["steps"], false);
      }
      ...
      return fetchMetricFromSupabaseByTypeNames(typeNames, useBP);
    },
    refetchInterval: 60000,
    refetchIntervalInBackground: true,
  });
